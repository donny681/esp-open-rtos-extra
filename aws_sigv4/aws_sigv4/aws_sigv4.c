...
aws_t aws_init(const char *region, const char *service, const char *host,
		const char *path, const char *http_method) {
...
//	bstring signed_headers = aws_headers_sign(context->form_data);
...
static int generate_string_to_sign(aws_t context) {
	...
	sha256_hex(hashed_canonical_request,
			(const char *) context->canonical_request->data);
...
int aws_sign(aws_t context, const char *secret, const char *date, char *out) {
	...
	hmac(kDate, kSecret->data, strlen((char *) kSecret->data),
			context->date.str_date_only->data,
			strlen((char *) context->date.str_date_only->data));
	hmac(kRegion, kDate, SHA256_SIZE, context->region->data,
			strlen((char *) context->region->data));
	hmac(kService, kRegion, SHA256_SIZE, context->service->data,
			strlen((char *) context->service->data));
	hmac(signing_key, kService, SHA256_SIZE,
			(unsigned char *) AWS_REQUEST_STRING, strlen(AWS_REQUEST_STRING));

	hmac_hex(signature, signing_key, SHA256_SIZE, context->string_to_sign->data,
			strlen((char *) context->string_to_sign->data));
...
static int parse_date(aws_t context, const char *str) {
	char yyyy_mm_dd_hh_mm_ss[17], yyyy_mm_dd[9];

	if (strptime(str, AWS_DATE_FORMAT, &context->date.tm) == NULL)
		return AWS_ERR;

	if (strftime(yyyy_mm_dd_hh_mm_ss, 17, "%Y%m%dT%H%M%SZ", &context->date.tm)
			== 0)
		return AWS_ERR;
	...
	printf(
			"context->date.original_str=%s\r\ncontext->date.str=%s\r\ncontext->date.str_date_only=%s\r\n",
			context->date.original_str->data, context->date.str->data,
			context->date.str_date_only->data);
	if (aws_add_header(context, "x-amz-date", context->date.str->data) != AWS_OK)
		return AWS_ERR;
...
static bool is_valid_http_method(const char *method) {
	...
	for (i = 0; i < HTTP_METHOD_COUNT; i++)
		if (strncmp(method, http_request_methods[i], HTTP_METHOD_MAX_LENGTH)
				== 0)
			return true;
...
static bool is_post_request(aws_t context) {
	return (bool)(
			strncmp((const char *) context->request_method->data,
					http_request_methods[1], HTTP_METHOD_MAX_LENGTH) == 0);
}

int shadow_aws_sign(aws_t context, const char *secret, const char *date,
		char *out) {
	bstring kSecret;
	unsigned char kDate[32], kRegion[32], kService[32], signing_key[32];
	char signature[65];
